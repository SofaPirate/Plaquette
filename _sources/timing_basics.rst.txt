...
you’re creating delays, triggering events, or generating periodic signals, Plaquette provides
powerful tools to manage time effectively. In this section, we will explore timing functions and
units, such as ``seconds``, :doc:`Chronometer`, :doc:`Alarm`, :doc:`Metronome`, and :doc:`Ramp`.

Timing is the backbone of interactive systems. By understanding and leveraging the timing tools
Plaquette provides, you can create precise, dynamic, and creative projects that respond in real-time
...
.. note::
  ...
  to visualize the ramps.

Measuring Absolute Time with seconds()
--------------------------------------

The most fundamental timing functionality in Plaquette is the ``seconds()`` function. It simply returns 
the elapsed time in seconds `since the program started running`. This is very useful for measuring 
durations or triggering time-based events.

.. warning::

    The value returned by ``seconds()`` is an approximation and should not be used as a substitute for 
    a real-time clock in applications requiring high precision.
...
While ``seconds()`` provides a simple and effective way to measure time, it is a global function
that cannot differentiate between multiple events: it simply tells you the time since the program was
started. Like time itself, it always increases and never resets. For more precise control and 
event-specific timing, we will need to use more specialized timing functionalities.
...
The :doc:`Chronometer` is great for tracking durations and reacting to them, but sometimes you want
an easy way to know whether you waited for a certain amount of time.
...
toggles LED visibility, while another slower metronome accelerates blinking speed.
...
Like :doc:`Alarm` units, ramps are restarted by calling their ``start()`` function. By default, they
...
making them useful for various applications such as changing properties of waves, controlling the 
...
The ``go()`` function provides a simple way to launch a ramp towards a new goal, starting from the
...
Plaquette allows you to combine timing units for more complex behaviors. For instance, you can use a 
:doc:`Metronome` to repeatedly trigger a :doc:`Ramp` or synchronize multiple timing units.
