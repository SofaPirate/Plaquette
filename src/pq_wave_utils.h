...
inline q0_32u_t triangleWave(q0_32u_t t, q0_32u_t skew)
{
  return (t <= skew) ?
            ...
            fixed32Divide(FIXED_MAX_32 - t, FIXED_MAX_32 - skew);
}
...
inline q0_32u_t sineWave(q0_32u_t t, q0_32u_t skew)
{
  ...
  {
      phase32 = fixed32Divide(t - skew, FIXED_MAX_32 - skew) / 2 + HALF_FIXED_MAX;
  }
  ...
  {
      phase16 = static_cast<uint16_t>((static_cast<uint64_t>(t - skew) << 15) / (FIXED_MAX_32 - skew)) + HALF_FIXED_MAX_16;
  }
  // Peak of sine wave.
  else
  { // t == _skew32
